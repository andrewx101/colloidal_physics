...
\begin{equation}
  \frac{\mathrm{D}}{\mathrm{D}t}f=-\symbf{\nabla}\cdot \mathbf{v}
\end{equation}
其中$\mathrm{D}/\mathrm{D}t=\partial/\partial t+\mathbf{v}\cdot\symbf{\nabla}$。可见，刘维尔方程\eqref{eq:Liouville equation}作为相空间中的流体的连续性方程，并不一般地保证“不可压缩性”。我们把$\symbf{\nabla}\cdot\mathbf{v}$展开可以写成
...
其中
\begin{align}
  \imagI \mathcal{L}f              & =\symbf{\nabla}\cdot\left(f\mathbf{v}\right)                                \\
                                   & =\mathbf{v}\cdot\symbf{\nabla}f+\left(\symbf{\nabla}\cdot\mathbf{v}\right)f \\
  \Leftrightarrow\imagI\mathcal{L} & =\mathbf{v}\cdot\symbf{\nabla}+\symbf{\nabla}\cdot\mathbf{v}
\end{align}
...
\begin{equation}
  \frac{\mathrm{d}}{\mathrm{d}t}\hat{B}\left(t\right)=\frac{\mathrm{d}}{\mathrm{d}t}B\left(\symbf{\Gamma}\left(t\right)\right)=\mathbf{v}\cdot\symbf{\nabla}B
\end{equation}
其中对$B$的梯度是在$\symbf{\Gamma}\left(t\right)$处求的，所以是沿着轨迹$\symbf{\Gamma}\left(t\right)$变化的。我们引入新的算符：
\begin{equation}
  \imagI L=\mathbf{v}\cdot\symbf{\nabla}
\end{equation}
...
\begin{equation}
  \frac{\mathrm{D}}{\mathrm{D}t}\tilde{B}\left(\symbf{\Gamma},t\right)=\frac{\partial}{\partial t}\tilde{B}\left(\symbf{\Gamma},t\right)+\mathbf{v}\cdot\symbf{\nabla}\tilde{B}=\mathbf{v}\cdot\symbf{\nabla}B
\end{equation}
...
\begin{proof}
  \begin{align*}
    lhs & =\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}f\left(\symbf{\Gamma},0\right)\mathbf{v}\cdot\symbf{\nabla}B\left(0\right)\mathrm{d}\symbf{\Gamma}                                                                                                                           \\
        & =-\int_{\partial\Lambda_0}\mathrm{d}\symbf{\Gamma}f\left(\symbf{\Gamma},0\right)B\left(0\right)\mathbf{v}\cdot\hat{\mathbf{n}}d\sigma+\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}B\left(0\right)\symbf{\nabla}\cdot\left(f\left(\symbf{\Gamma},0\right)\mathbf{v}\right) \\
        & =-\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}B\left(0\right)\imagI \mathcal{L}f\left(\symbf{\Gamma},0\right)=rhs
  \end{align*}
  其中第二个等号第一项因在$\Lambda_0$的边界上总有$f\left(\symbf{\Gamma},0\right)\to 0$而为零，因为物理上我们常进一步要求$f$满足狄利克雷条件，因此$f$绝对可积，结合它是恒正且满足归一化条件的要求，充分地它在$\Lambda_0$的边界上总有$f\left(\symbf{\Gamma},0\right)\to 0$。剩下的第二项变到第三个等号时用到了$-\imagI\mathcal{L}f\left(\symbf{\Gamma},0\right)=\symbf{\nabla}\cdot\left(f\left(\symbf{\Gamma},0\right)\mathbf{v}\right)$。
\end{proof}
...
\begin{proof}
  \begin{align*}
    \left\langle B\right\rangle_\text{H}\left(t\right) & =\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}f\left(\symbf{\Gamma},0\right)\exp\left(\imagI Lt\right)B\left(\symbf{\Gamma}\right)                                                                                                 \\
                                                       & =\sum_{n=0}^\infty\frac{1}{n!}\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}f\left(\symbf{\Gamma},0\right)\left(t\mathbf{v}\cdot\symbf{\nabla}\right)^nB\left(\symbf{\Gamma}\right)                                                 \\
                                                       & =\sum_{n=0}^\infty\frac{1}{n!}\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}f\left(\symbf{\Gamma},0\right)\imagI L\left[\left(t\mathbf{v}\cdot\symbf{\nabla}\right)^{n-1}B\left(\symbf{\Gamma}\right)\right]                        \\
                                                       & =\sum_{n=0}^\infty\frac{1}{n!}\int_{\Lambda_0}\mathrm{d}\symbf{\Gamma}\left(-\imagI\mathcal{L}f\left(\symbf{\Gamma},0\right)\right)\left[\left(t\mathbf{v}\cdot\symbf{\nabla}\right)^{n-1}B\left(\symbf{\Gamma}\right)\right] \\
                                                       & \vdots                                                                                                                                                                                                                        \\
...
我们看到，$\left\langle B\right\rangle\left(t\right)$的时间依赖性其实是来自概率密度函数$f\left(\symbf{\Gamma},t\right)$的时间依赖性。我们常常看到，体系的宏观性质在外界条件恒定的情况下，假以时日总是趋于稳定，不再依赖时间（趋近平衡）。这反映$f\left(\symbf{\Gamma},t\right)$在$t\to \infty$时失去时间依赖性，从而有
\begin{equation}
  f_\text{eq}\left(\symbf{\Gamma}\right)=f\left(\symbf{\Gamma},t\to\infty\right)
\end{equation}
或者说，
故
\begin{equation}
...
\end{equation}
其中下标$t$表示这是一个时间平均，而非系综平均。对于稳态下的体系，如果有$\left\langle B\right\rangle_t=\left\langle B\right\rangle$，就称这个系统是\emph{遍历的（ergodic）}。在一般平衡态统计力学书中，系统的遍历性是公理化规定的。但是在非平衡态统计力学与动力学系统的交叉领域，人们倾向于不直接规定遍历性，而希望从微观动力学出发得出系统是否遍历或何时遍左边。相关的话题不在此展开\footnote{参见文献}。
\end{document}